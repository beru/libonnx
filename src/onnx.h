...
struct onnx_tensor_t {
	...
	int indices_to_offset(const int * indices)
	{
		int offset = 0;
	...
	void offset_to_indices(int offset, int * indices)
	{
...
static inline bool onnx_tensor_is_scalar(const onnx_tensor_t * t)
{
	return ((t->ndim == 0) && (t->ndata == 1));
}

static inline bool onnx_tensor_broadcast_is_valid(const onnx_tensor_t * x, int * dims, int ndim)
{
	int i;
...
static inline void * onnx_tensor_broadcast_map_address(onnx_tensor_t * x, onnx_tensor_t * y, int offset)
{
	...
	if((xndim > 0) && (yndim > 0))
	{
		...
		return (char*)x->datas + x->indices_to_offset(&ix[0]) * onnx_tensor_type_sizeof(x->type);
	}
	return x->datas;
}
