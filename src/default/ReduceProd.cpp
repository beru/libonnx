...
static void ReduceProd_generic(struct onnx_node_t * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	struct onnx_tensor_t * x = n->inputs[0];
	struct onnx_tensor_t * y = n->outputs[0];
...
static void ReduceProd_bfloat16(struct onnx_node_t * n)
{
	...
	do
	{
		...
		do
		{
			prod *= bfloat16_to_float32(px[o + dim_offset(pdat->naxes, &iter_in_axes[0], &in_axes_axis_dis[0])]);
		} while(dim_next(pdat->naxes, &iter_in_axes[0], &iter_in_axes_max[0]));
...
static void ReduceProd_float16(struct onnx_node_t * n)
{
	...
	do
	{
		...
		do
		{
			prod *= float16_to_float32(px[o + dim_offset(pdat->naxes, &iter_in_axes[0], &in_axes_axis_dis[0])]);
		} while(dim_next(pdat->naxes, &iter_in_axes[0], &iter_in_axes_max[0]));
